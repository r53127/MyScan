...
class ExamControl():
    ...
    def startMarking(self, pic):#自适应阈值批量阅卷并做各种记录和保存处理
        ...
        try:
            ...
            #自动适应阈值阅卷
            failedCount, successedCount,self.markingResult,confirmResult= self.autoScan(pic, failedCount, successedCount)
            # 记录该文件阅卷结果
    ...
    def autoScan(self, pic, failedCount, successedCount):#自适应阈值扫描
        ...
        for a in range(2, 8):  # 程序自行尝试调节阈值
            ...
            self.markingResult = self.marking(pic)  # 阅卷
            QApplication.processEvents()  # 停顿刷新界面
        ...
        if retry_flag == 1:#手动调节阈值
            # 如果程序调节失败，操作者自行调节阈值
            confirmResult= self.confirmMarking(pic)
